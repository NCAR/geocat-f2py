...
c        if (DEBUG) then
c            print *, 'entering deof with nx, nt, nmodes=',nx,nt,nmodes
...
c	---------------------------------------------------------
c        if (DEBUG) then
c            print *, 'figuring switched or not'
c        end if

	doswitched = (nx .gt. nt)
	if( doswitched ) then
		orderofs = nt
c		if (DEBUG) print *, 'deof: Working in switched mode'
	else
		orderofs = nx
c		if (DEBUG) print *, 'deof: Working in unswitched mode'
	endif
	if( orderofs .gt. iminnxnt ) then
c		print * , 'Error!  EOF routine must be supplied '
c		print * , 'with enough workspace; passed parameter'
c		print * , 'iminnxnt must be at least ', orderofs
c		print * , 'Passed value was iminnxnt=',iminnxnt
		stop 'eof '
	endif
	if( nmodes .gt. orderofs ) then
c		print * , 'Error! EOF routine called requesting more'
c		print * , 'modes than exist!  Request=',nmodes,' exist=',
c     &		           orderofs
		stop 'eof '
	endif
...
c	------------------------------
	if( info .ne. 0 ) then
		if( info .lt. 0 ) then
c			print *, 'LAPACK error: argument ',
c     &				-info, ' had illegal value'
			stop 'eof'
		else
c			print *, 'LAPACK error: ', info,
c     &				'eigenvectors failed to converge!'
c			print *, 'Consult the LAPACK docs!'
			stop 'eof'
		endif
	endif
...
c	    print *, 'Warning! LAPACK routine returned eigenvalue <= 0.'
c	    print *, 'Setting it to zero...'
            evals(i) = 0.0d0
c	    do j=1, nmodes
c	      print *, j, evals(j)
c  	    end do
...
c	---------------
	...
	if( nx .le. 1 ) then
c		print *, 'covariance: error: nx too small!! nx=', nx
c c c           call exit(-1)
                stop
	endif
	if( doswitched ) then
