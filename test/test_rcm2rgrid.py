...
class Test_rcm2rgrid(ut.TestCase):
    ...
    def test_rcm2rgrid_float64_nom(self):
        nt.assert_array_almost_equal(
            fo_nom_expected,
            gctmp.rcm2rgrid(lat2d, lon2d, fi_nom.astype(np.float64), lat, lon))

    def test_rcm2rgrid_float64_nan(self):
        nt.assert_array_almost_equal(
            fo_nan_expected,
            gctmp.rcm2rgrid(lat2d, lon2d, fi_nan.astype(np.float64), lat, lon))

    def test_rcm2rgrid_float64_msg(self):
        nt.assert_array_almost_equal(
            fo_msg_expected,
            gctmp.rcm2rgrid(lat2d,
                            lon2d,
    ...
    def test_rcm2rgrid_float32_nom(self):
        nt.assert_array_almost_equal(
            fo_nom_expected,
            gctmp.rcm2rgrid(lat2d, lon2d, fi_nom.astype(np.float32), lat, lon))

    def test_rcm2rgrid_float32_nan(self):
        nt.assert_array_almost_equal(
            fo_nan_expected,
            gctmp.rcm2rgrid(lat2d, lon2d, fi_nan.astype(np.float32), lat, lon))
